...
use App\Http\Requests\StoreOneTagDispersionRequest;
use App\Http\Requests\StoreManyTagDispersionRequest;
use App\Http\Requests\UpdateOneTagDispersionRequest;
use Illuminate\Http\Request;
use App\Models\TagDispersion;
use Illuminate\Support\Facades\DB;
use App\Models\Vehicle;

class TagDispersionController extends Controller
{
    public function storeOne(StoreOneTagDispersionRequest $request)
    {
        $data = $request->validated();

        try {
            $tag_dispersion = TagDispersion::create($data);
        } catch (\Exception $e) {
            return back()->withErrors(['error' => 'No se pudo registrar la dispersión de caseta: ' . $e->getMessage()])
                ->withInput();
        }

        return redirect()->route('dispersiones.caseta_disp_consulta_act', ['dispersion' => $tag_dispersion->id])
            ->with('success', 'Dispersión de caseta registrada exitosamente ;).');
    }

    public function storeMany(StoreManyTagDispersionRequest $request)
    {
    ...
    public function show(TagDispersion $dispersion)
    {
        return view('Gestion_dispersiones_monetarias.gdm_caseta_disp_consulta_act', ['dispersion' => $dispersion, 'vehicles' => Vehicle::all()]);
    }

    public function update(UpdateOneTagDispersionRequest $request)
    {
        $data = $request->validated();

        try {
            // Buscar la dispersión existente.
            $dispersion = TagDispersion::findOrFail($request->input('id'));

            // Intentar actualizar.
            $dispersion->update($data);

            return redirect()->route('dispersiones.caseta_disp_consulta_act', ['dispersion' => $dispersion->id])
                ->with('success', 'Dispersión de caseta actualizada exitosamente ;).');
    ...
    public function find(Request $request)
    {
        ...
        try {
            ...
            $query = TagDispersion::orderBy('fecha_dispersion', 'desc')->limit(50);
            ...
            return response()->json([
                'err' => 'Error en el sistema SGVAP al consultar las dispersiones de caseta: ' . $e->getMessage(),
            ], 500);
        }
    }
    ...
    {
        try {
            $tag_dispersion = TagDispersion::findOrFail($id);

            $tag_dispersion->delete();
        } catch (\Exception $e) {
            return redirect()->route('dispersiones.caseta_alta_dispersion')->withErrors(['error' => 'No se pudo eliminar la dispersión de caseta: ' . $e->getMessage()]);
        }

        return redirect()->route('dispersiones.caseta_alta_dispersion')->with('success', 'Dispersión de caseta eliminada exitosamente ;).');
    }
}
