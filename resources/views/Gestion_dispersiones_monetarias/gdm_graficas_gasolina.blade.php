...
@section('content')
    <div class="w-100 my-3 div-main">
        ...
        <div class="w-100 div-secondary px-5 py-5 d-block">
            ...
            <form id="generar_graficas_viatico_gasolina" action="{{ route('dispersiones.graficas_gasolina') }}" method="get"
                ...
                <div class="row g-3">
                    ...
                    <button class="d-block mx-auto btn btn-primary btn-lg fw-bold button-custom" type="button"
                        onclick="ask_before_submit_new('generar_graficas_viatico_gasolina')" style="background-color: var(--botones-color);">Generar</button>
    ...
    @php
        $data_projects_Array_yValues = $gasolina_por_proyecto
            ->pluck('total_invertido_gasolina')
            ->map(fn($value) => (float) $value)
            ->toArray();
        $data_vehicles_Array_yValues = $gasolina_por_vehiculo
            ->pluck('total_invertido_gasolina')
            ->map(fn($value) => (float) $value)
            ->toArray();

        $data_projects_Array_xValues = $gasolina_por_proyecto->pluck('project_name')->toArray();
        $data_vehicles_Array_xValues = $gasolina_por_vehiculo->pluck('vehicle_id')->toArray();
    @endphp
    ...
    <div class="d-flex gap-2 justify-content-between align-items-stretch px-1 flex-wrap">
        <div class="p-3 d-inline-block rounded-3 do_responsive_div3 mx-auto"
            ...
            @if ($gasolina_por_proyecto->isNotEmpty())
                <canvas id="myChartProjects_Gasoline" class="d-block mx-auto" data-yValues='@json($data_projects_Array_yValues)'
        ...
        <div class="p-3 d-inline-block rounded-3 do_responsive_div3 mx-auto"
            style="box-shadow: 0px 0px 8px 0px rgba(0,0,0,0.75) inset;">
            @if ($gasolina_por_vehiculo->isNotEmpty())
                <canvas id="myChartVehicles_Gasoline" class="d-block mx-auto" data-yValues='@json($data_vehicles_Array_yValues)'
